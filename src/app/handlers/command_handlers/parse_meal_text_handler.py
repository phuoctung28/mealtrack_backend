    ...
    async def _translate_english_names(
        ...
        try:
            ...
            with sentry_sdk.start_span(op="gen_ai.request", name="translate_meal_names") as span:
                span.set_data("gen_ai.request.model", trans_model_name)
                ...
                if isinstance(trans_usage, dict):
                    if trans_usage.get("input_tokens") is not None:
                        span.set_data("gen_ai.usage.input_tokens", trans_usage["input_tokens"])
                    if trans_usage.get("output_tokens") is not None:
                        span.set_data("gen_ai.usage.output_tokens", trans_usage["output_tokens"])
