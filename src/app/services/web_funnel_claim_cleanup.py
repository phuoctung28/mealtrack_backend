...
async def cleanup_expired_reservations(
    db: AsyncSession, firebase: WebFunnelFirebaseIdentityService | None = None, limit: int = 25
) -> int:
    ...
    claims = (await db.scalars(select(WebFunnelClaim).where(WebFunnelClaim.reservation_uid.is_not(None), WebFunnelClaim.reservation_expires_at < utcnow(), WebFunnelClaim.consumed_at.is_(None), WebFunnelClaim.revoked_at.is_(None)).limit(limit).with_for_update(skip_locked=True))).all()
    removed = 0
    for claim in claims:
