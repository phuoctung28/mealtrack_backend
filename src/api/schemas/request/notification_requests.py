...
class NotificationPreferencesUpdateRequest(BaseModel):
    ...
    daily_summary_time_minutes: Optional[int] = Field(
        None, ge=0, le=1439, description="Daily summary time (minutes from midnight)"
    )

    # Preferred notification language (ISO 639-1)
    language: Optional[str] = Field(
        None, max_length=5, description="Preferred notification language (ISO 639-1)"
    )
    ...
    def validate_language(cls, v):
        ...
        if v is not None and v not in SUPPORTED_NOTIFICATION_LANGUAGES:
            raise ValueError(
                f"language must be one of: {SUPPORTED_NOTIFICATION_LANGUAGES}"
            )
        return v
