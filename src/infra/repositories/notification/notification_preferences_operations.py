...
class NotificationPreferencesOperations:
    ...
    def save_notification_preferences(
        db: Session, preferences: NotificationPreferences
    ) -> NotificationPreferences:
        """Save notification preferences to the database."""
        try:
            existing_prefs = (
                db.query(NotificationPreferencesORM)
                .filter(NotificationPreferencesORM.user_id == preferences.user_id)
                .first()
            )

            if existing_prefs:
                existing_prefs.meal_reminders_enabled = (
                ...
                existing_prefs.daily_summary_time_minutes = (
                    preferences.daily_summary_time_minutes
                )
    ...
    def find_notification_preferences_by_user(
        ...
        db_prefs = (
            db.query(NotificationPreferencesORM)
            .filter(NotificationPreferencesORM.user_id == user_id)
            .first()
        )
