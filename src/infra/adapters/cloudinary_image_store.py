...
class CloudinaryImageStore(ImageStorePort):
    ...
    def save(self, image_bytes: bytes, content_type: str, image_id: Optional[str] = None) -> str:
        ...
        """
        logger.debug(f"Saving image of type {content_type}, size {len(image_bytes)} bytes")
        ...
        try:
            ...
            if (response_url):
                ...
                logger.warning(f"'secure_url' not found in Cloudinary response. Returning fallback image_id: {image_id}")
                return image_id
