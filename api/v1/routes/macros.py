...
@router.get("/daily/entries", response_model=List[Dict])
async def get_daily_meal_entries(
    ...
    - Returns list of meals consumed on the given date
    - Defaults to today if no date provided
    """
    try:
        from datetime import datetime, date as date_obj
        ...
        logger.info(f"Retrieving meal entries for date: {parsed_date}")
        
        # Get actual meals from database for the specified date
        meals = meal_handler.get_meals_by_date(parsed_date)
        
        # Convert meals to API response format
        entries = []
        for meal in meals:
            # Only include meals that have completed nutrition analysis
            if meal.status != MealStatus.READY or not meal.nutrition:
                continue
                
            # Create entry from meal data
            entry = {
                "id": meal.meal_id,
                "name": f"Meal from {meal.created_at.strftime('%H:%M')}",
                "calories": meal.nutrition.calories,
                "macros": {
                    "protein": meal.nutrition.macros.protein,
                    "carbs": meal.nutrition.macros.carbs,
                    "fat": meal.nutrition.macros.fat,
                    "fiber": meal.nutrition.macros.fiber or 0
                },
                "timestamp": meal.created_at.isoformat(),
                "meal_type": _determine_meal_type(meal.created_at.time()),
                "description": _generate_meal_description(meal),
                "image_url": meal.image.url if hasattr(meal.image, 'url') and meal.image.url else None,
                "quantity": 1.0,
                "unit": "serving",
                "confidence_score": meal.nutrition.confidence_score or 0.8
            }
        ...
        logger.info(f"Found {len(entries)} meal entries for date {parsed_date}")
        return entries
        
    except Exception as e:
        logger.error(f"Error retrieving daily meal entries: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error retrieving daily meal entries: {str(e)}"
        )
...
def _generate_meal_description(meal):
    """Generate a description for the meal based on its food items."""
    if not meal.nutrition or not meal.nutrition.food_items:
        return f"Meal from {meal.created_at.strftime('%H:%M')}"
    
    # Get the first few food items to create a description
    food_names = [item.name for item in meal.nutrition.food_items[:3]]
    
    if len(food_names) == 1:
        return food_names[0]
    elif len(food_names) == 2:
        return f"{food_names[0]} and {food_names[1]}"
    elif len(food_names) == 3:
        return f"{food_names[0]}, {food_names[1]} and {food_names[2]}"
    else:
        return f"{food_names[0]}, {food_names[1]} and others"
