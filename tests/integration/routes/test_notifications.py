...
class TestNotificationRequest(BaseModel):
    ...
    notification_type: str = Field(
        default="test", description="Type of test notification"
    )
...
async def send_test_notification(
    ...
    try:
        ...
        result = await scheduled_service.send_test_notification(
            user_id=request.user_id, notification_type=request.notification_type
        )
...
async def get_notification_status(
    ...
    try:
        firebase_initialized = (
            firebase_service.is_initialized() if firebase_service else False
        )
        scheduled_service_running = (
            scheduled_service.is_running() if scheduled_service else False
        )
        ...
        return {
            ...
            "message": "Notification system is "
            + ("running" if scheduled_service_running else "not running"),
            "status": (
                "healthy"
                if (firebase_initialized and scheduled_service_running)
                else "degraded"
            ),
        }
