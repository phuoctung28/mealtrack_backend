...
def api_client(test_session) -> Generator[TestClient, None, None]:
    ...
    class MockImageStore(ImageStorePort):
        ...
        async def get_url_async(self, image_id):
            return self.get_url(image_id)

        async def delete_async(self, image_id):
    ...
    from src.infra.adapters.meal_generation_service import (
        MealGenerationService as OriginalMealGen,
    )
            ...
            if schema and hasattr(schema, "__name__") and "Name" in schema.__name__:
                ...
                return {
                    ...
                    "recipe_steps": [
                        {
                            "step": 1,
                            "instruction": "Grill chicken breast for 6 minutes per side",
                            "duration_minutes": 6,
                        },
                        {
                            "step": 2,
                            "instruction": "Chop lettuce and tomatoes",
                            "duration_minutes": 5,
                        },
                        {
                            "step": 3,
                            "instruction": "Serve chicken over salad with olive oil dressing",
                            "duration_minutes": 2,
                        },
                    ],
                }
...
def mock_services():
    ...
    mock_image_store.upload_image = AsyncMock(
        return_value="https://mock-image-url.com/image.jpg"
    )
