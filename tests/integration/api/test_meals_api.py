...
class TestMealsAPI:
    ...
    def test_analyze_meal_image_success(
        self, authenticated_client, sample_image_bytes, test_session
    ):
    ...
    def test_analyze_meal_image_with_target_date(
        self, authenticated_client, sample_image_bytes, test_session
    ):
        ...
        response = authenticated_client.post(
            f"/v1/meals/image/analyze?target_date={target_date}", files=files
        )
    ...
    def test_analyze_meal_image_invalid_file_type(self, authenticated_client):
        ...
        assert "Invalid file type" in data["detail"][
            "message"
        ] or "Invalid file type" in str(data["detail"])
    ...
    def test_analyze_meal_image_file_too_large(self, authenticated_client):
        ...
        assert "File size exceeds" in data["detail"][
            "message"
        ] or "File size exceeds" in str(data["detail"])
    ...
    def test_analyze_meal_image_by_url_success(
        ...
        with patch(
            ...
            response = authenticated_client.post(
                "/v1/meals/image/analyze-url", json=payload
            )
    ...
    def test_create_manual_meal_success(
        self, authenticated_client, test_user, test_session
    ):
    ...
    def test_edit_meal_ingredients(self, authenticated_client, test_user, test_session):
        ...
        with patch(
            ...
            response = authenticated_client.put(
                f"/v1/meals/{meal.meal_id}/ingredients", json=payload
            )
    ...
    def test_delete_meal_not_found(self, authenticated_client):
        ...
        with patch(
            ...
            mock_bus.send = AsyncMock(
                side_effect=ResourceNotFoundException(f"Meal {fake_meal_id} not found")
            )
    ...
    def test_get_daily_macros(
        ...
        for i in range(2):
            ...
            db_image = MealImageORM(
                image_id=image_id, format="jpeg", size_bytes=102400, url=None
            )
        ...
        response = authenticated_client.get(
            f"/v1/meals/daily/macros?date={target_date}"
        )
        ...
        assert (
            "target_calories" in data or "target_macros" in data
        )  # May or may not have targets

    def test_get_daily_macros_without_date(
        ...
        with patch(
            ...
            mock_response = {
                ...
                "target_macros": {"protein": 150.0, "carbs": 200.0, "fat": 65.0},
                "total_calories": 0.0,
                "total_protein": 0.0,
                "total_carbs": 0.0,
                "total_fat": 0.0,
