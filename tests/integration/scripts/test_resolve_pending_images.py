...
async def test_drain_processes_pending_rows(db_session, monkeypatch):
    ...
    _script = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "..", "..", "..", "scripts", "resolve_pending_images.py")
    _spec = importlib.util.spec_from_file_location("resolve_pending_images", _os.path.abspath(_script))
    ...
    await pending_repo.enqueue_many([
        ...
        PendingItem(
            meal_name="Pizza Margherita", name_slug="pizza-margherita",
