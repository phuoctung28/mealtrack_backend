...
class TestMealEditAPI:
    ...
    @pytest.mark.asyncio
    async def test_update_meal_ingredients_success(
        self, client, sample_meal_with_nutrition
    ):
        ...
        request_data = {
            ...
            "food_item_changes": [
                {"action": "update", "id": id, "quantity": 200.0, "unit": "g"}
            ],
        }

        # Act
        response = client.put(
            f"/v1/meals/{meal.meal_id}/ingredients", json=request_data
        )
    ...
    @pytest.mark.asyncio
    async def test_update_meal_ingredients_add_custom(
        self, client, sample_meal_with_nutrition
    ):
    ...
    @pytest.mark.asyncio
    async def test_update_meal_ingredients_remove(
        self, client, sample_meal_with_nutrition
    ):
        ...
        request_data = {"food_item_changes": [{"action": "remove", "id": id}]}
    ...
    async def test_update_meal_unauthorized(self, client, sample_meal_with_nutrition):
        ...
        # Act
        response = client.put(
            "/v1/meals/non-existent-meal-id/ingredients", json=request_data
        )
    ...
    async def test_update_meal_nonexistent(self, client):
        ...
        # Act
        response = client.put(
            "/v1/meals/non-existent-meal/ingredients", json=request_data
        )
    ...
    @pytest.mark.asyncio
    async def test_update_meal_invalid_request_data(
        self, client, sample_meal_with_nutrition
    ):
    ...
    @pytest.mark.asyncio
    async def test_multiple_changes_in_one_request(
        self, client, sample_meal_with_nutrition
    ):
        ...
        request_data = {
            ...
            "food_item_changes": [
                ...
                {"action": "remove", "food_item_id": food_item_2_id},
                {
                    "action": "add",
                    "name": "Fresh Herbs",
                    "quantity": 10.0,
        ...
        updated_food_items = result["updated_food_items"]
        herb_item = next(
            (item for item in updated_food_items if item["name"] == "Fresh Herbs"), None
        )
