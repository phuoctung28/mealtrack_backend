...
def client(test_session):
    ...
    def override_get_configured_event_bus():
        ...
        with patch(
            "src.api.base_dependencies.get_suggestion_orchestration_service",
            side_effect=mock_get_suggestion_service,
        ):
            with patch(
                "src.api.base_dependencies.get_cache_service",
                return_value=mock_cache_service,
            ):
                with patch(
                    "src.api.base_dependencies.get_food_cache_service",
                    return_value=mock_food_cache,
                ):
                    from src.api.dependencies.event_bus import (
                        get_configured_event_bus as real_get_bus,
                    )
...
def setup_test_user(test_session):
    ...
    profile = UserProfile(
        ...
        - timedelta(days=10),  # Old enough for goal changes
    )
