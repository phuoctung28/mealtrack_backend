...
def test_connection_pool_has_socket_timeout():
    ...
    with patch("src.infra.cache.redis_client.redis.ConnectionPool.from_url") as mock_from_url:
        mock_from_url.return_value = MagicMock()
        RedisClient(redis_url="redis://localhost:6379")
        call_kwargs = mock_from_url.call_args[1]
        assert "socket_timeout" in call_kwargs, "socket_timeout must be set"
        assert "socket_connect_timeout" in call_kwargs, "socket_connect_timeout must be set"
...
def test_connection_pool_timeout_custom_value():
    ...
    with patch("src.infra.cache.redis_client.redis.ConnectionPool.from_url") as mock_from_url:
        mock_from_url.return_value = MagicMock()
        RedisClient(redis_url="redis://localhost:6379", socket_timeout=1.5, socket_connect_timeout=2.5)
        call_kwargs = mock_from_url.call_args[1]
