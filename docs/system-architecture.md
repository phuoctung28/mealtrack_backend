...
4. `swap`, `log`, and `skip` return the changed-slot detail shape so the mobile client can patch its cached plan without reloading everything. First-time catalog `log` materialization schedules profile-aware meal value insights as a best-effort post-commit step, matching scan and manual-save warmup. Idempotent log replay does not reschedule.
